 ...
 * `history.state` is a `JsAny?`, so the depth string must be bridged across the Wasm/JS boundary
 * via [toJsString] when writing and cast back through [JsString] when reading. The js target
 * instead uses plain `kotlin.Any?` and needs no bridging. The shared `webMain` source set can't
 * express both, so the read/write of `history.state` is delegated to these per-target actuals.
 */
